...
class sherwood_v10_table : private EntryAlloc,
 ...
 public:
  ...
  sherwood_v10_table(
      ...
      : sherwood_v10_table(first, last, bucket_count, ArgumentHash(),
            ArgumentEqual(), alloc) {}
  template <typename It>
  ...
  sherwood_v10_table(const sherwood_v10_table &other)
      : sherwood_v10_table(
            other, AllocatorTraits::select_on_container_copy_construction(
                       other.get_allocator())) {}
  sherwood_v10_table(
      const sherwood_v10_table &other, const ArgumentAlloc &alloc)
      : EntryAlloc(alloc),
        Hasher(other),
        Equal(other),
  ...
  sherwood_v10_table(
      sherwood_v10_table &&other, const ArgumentAlloc &alloc) noexcept
      : EntryAlloc(alloc),
        Hasher(std::move(other)),
        Equal(std::move(other)),
        BucketAllocator(alloc),
    ...
    *new_buckets = EntryPointer(nullptr) + ptrdiff_t(1);
    ...
    for (EntryPointer *it = new_buckets,
      ...
      for (EntryPointer e = *it; e;) {
        ...
        size_t index = hash_policy.index_for_hash(
            hash_object(e->value), num_slots_minus_one);
  ...
  convertible_to_iterator erase(const_iterator to_erase) {
    ...
    AllocatorTraits::destroy(
        *this, std::addressof(to_erase.current_element->value));
  ...
  convertible_to_iterator erase(
      const_iterator begin_it, const_iterator end_it) {
  ...
  void swap(sherwood_v10_table &other) {
    ...
    swap_pointers(other);
    swap(
        static_cast<ArgumentHash &>(*this), static_cast<ArgumentHash &>(other));
...
class unordered_map
    : public detailv10::sherwood_v10_table<std::pair<K, V>, K, H,
          detailv10::KeyOrValueHasher<K, std::pair<K, V>, H>, E,
          ...
          typename std::allocator_traits<A>::template rebind_alloc<
              typename std::allocator_traits<A>::template rebind_traits<
                  detailv10::sherwood_v10_entry<std::pair<K, V>,
                      A>>::pointer>> {
  using Table = detailv10::sherwood_v10_table<std::pair<K, V>, K, H,
      detailv10::KeyOrValueHasher<K, std::pair<K, V>, H>, E,
      detailv10::KeyOrValueEquality<K, std::pair<K, V>, E>, A,
      typename std::allocator_traits<A>::template rebind_alloc<
          detailv10::sherwood_v10_entry<std::pair<K, V>, A>>,
...
class unordered_set
    : public detailv10::sherwood_v10_table<T, T, H,
          detailv10::functor_storage<size_t, H>, E,
  ...
  using Table = detailv10::sherwood_v10_table<T, T, H,
      detailv10::functor_storage<size_t, H>, E,
      detailv10::functor_storage<bool, E>, A,
      typename std::allocator_traits<A>::template rebind_alloc<
          detailv10::sherwood_v10_entry<T, A>>,
